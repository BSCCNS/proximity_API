...
categ = ['culture_foot',
       'education_foot', 'healthcare_foot', 'outdoor_foot', 'physical_foot',
       'restaurant_foot', 'services_foot', 'supplies_foot', 'transport_foot']
...
def main(pedestrian, proximity, sdemo):
    ...
    drop_cols = [col for col in proximity_expanded.columns if type(proximity_expanded[col].values[0]) != np.float64 and type(proximity_expanded[col].values[0]) != float]     
    aggdict = {col: agg(col) for col in proximity_expanded.columns if col not in drop_cols+['geom']}
